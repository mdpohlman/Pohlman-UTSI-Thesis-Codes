...
%% File Path
path = 'C:\Users\mpohlma3\Documents\Roughness Sleeves\FLDI Data\';
% % % Photodiode Data % % %
filename = 'C1Smooth8.mat';
filename2 = 'C2Smooth8.mat';
File = [path,filename];
...
%% Freestream Data
FS_filename = 'C1Freestream5.mat';
FS_file = [path,FS_filename];
FS_Data = importdata(FS_file);
FS_MS = FS_Data(160000:400000,:)-mean(FS_Data(160000:400000,:));

%% Temporal Data
% % % Import Data % % %
Data1 = importdata(File);
MSData1 = Data1-mean(Data1);
MSData1 = highpass(MSData1(160000:400000,:),1e4,fs);
time = ((1:length(MSData1(:,1)))./fs).*1000;   %(ms)

Data2 = importdata(File2);
MSData2 = Data2-mean(Data2);
MSData2 = highpass(MSData2(160000:400000,:),1e4,fs);
...
[PSD,F,Gxxfs,Ffs,Gxx,dF,dt,Velocity,Lags,Coef] = DiodeAnalysis(MSData1,MSData2,FS_MS,Samples,fs,BeamSep);
...
% % % Photodiode Analysis % % %
function [PSD,F,Gxxfs,Ffs,Gxx,dF,dt,Velocity,Lags,Coef] = DiodeAnalysis(MSData1,MSData2,FS_MS,Samples,fs,BeamSep)
...
% % % Model Run % % %
[PSD,F] = pwelch(MSData1,Win,[],[],fs);

dF = F(2)-F(1);

Gxx = PSD.*dF./var(MSData1);

%% Diode Velocity
% % % Model % % %
[Coef,Lags] = xcorr(MSData1(:,1),MSData2(:,1),'coef');
[~,loc] = max(abs(Coef));
dt = abs(Lags(loc)/fs);
